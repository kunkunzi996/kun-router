...
flowchart TD
    ...
    READY -- 是 --> PL[🚀 流水线收口 19<br/>初验→commit→同步基准分支并复验→push→PR→CI→合并→同步主工作区→部署→线上冒烟]
    MANUAL --> PL
    PL --> RPT[汇报结果]
    RPT --> CONF[等用户确认没问题]
    CONF --> CLEAN[终态清理<br/>列出精确清单<br/>worktree 按项目规则由用户手动清理]
    CLEAN --> L{需要收尾/交接吗}
    P -- 否 --> L
...
- 开了 feature 分支的任务，初验后先保存提交，再同步基准分支并重新验一遍；复验通过才推分支、发 PR、等 CI、合并、同步主工作区和上线冒烟。自动挡只有契约完整才连跑，缺字段会降级。你确认精确清理清单后才处理分支，worktree 是否由 AI 移除服从项目删除规则。
...
flowchart TD
    ...
    Q2{开工门禁<br/>要不要过？} -- 新项目首次施工/<br/>大功能/没档案 --> GATE[🚧 完整 7 项体检<br/>1.目录清楚<br/>2.MVP锁定<br/>3.致命卡点<br/>4.有Spec<br/>5.Git初始化<br/>6.分支与工作区<br/>7.挡位与准入]
    GATE --> GATEOK{全过了吗}
