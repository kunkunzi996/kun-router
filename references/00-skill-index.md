...
1. 流水线收口（19）
2. 保存汇报（如本轮还没验收，先回 12）

自动挡先做契约与工具权限检查，缺失时降级手动挡；禁止无 `gh` 时偷偷改走本地合并。
...
- 只是小 UI / 文案：手动挡跳过开工门禁；自动挡只跳过完整门禁，仍运行 03 第 6/7 项轻量门禁。两种挡位都跳过开工规格、执行规格和架构门。
...
手动挡启用安全施工 + 最小验收；自动挡在此基础上前置 03 第 6/7 项、后接 19，禁止恢复全套重流程。
