...
正式施工前必须先确认这次改动在哪个分支上做，没确认前不许写代码。

判断规则：

- 新功能 / 大改 / 影响多个文件或页面 → 新开 feature 分支：

```bash
git checkout -b feature/本次功能名
```

- 小修 / 小 bug / 单文件小改 / 改文案或样式 → 直接在当前分支，不用新建。

强制三选一（用户不选不施工）：

1. 新建 `feature/xxx` 分支（推荐用于新功能 / 大改）
2. 就在当前分支继续（适合小修小补）
3. 我已经在正确的分支上了

对新手解释：

> 分支就像草稿本里单开的一页。新功能单开一页，写坏了整页撕掉也不弄脏正本（main）；小修小补直接在当前页改，省事。

## 建议补充：AI 会话角色是否分工

> 这是**建议项**，不是必查 6 项之一，不强制卡住施工（通过标准里也不含此项）。复杂项目建议做，单文件小修可跳过。
...
- 当前 Spec 已存在或正在生成。
- Git 状态清楚。
- 分支策略已确认（新建 feature 分支 / 当前分支 / 不适用）。
- 本轮只做 / 不做已写明。
