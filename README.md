...
- **V0.9.0（本版）**：项目洁癖三层知识编辑拆为独立 Skill `kun-cleanup-gate`，直接调用不再经 Router；Router 保留移交入口，两侧 description 划清边界。
- **V0.8.2**：执行规则收紧——后端哨兵只在未验收后端会被本轮业务使用时触发；所有挡位精确暂存；轻量 UI 的具体步骤只以 16 号为准；自动挡同步后以代码树判断是否需要重跑完整验收。
- **V0.8.1**：路由一致性与验收减负——16 号成为任务顺序唯一来源，门禁改用语义名称；验证按 V1 终端、V2 浏览器、V3 Computer-Use 分级，V3 仅在前两级不足且本轮已授权时启用；低风险小改强制使用 6 项轻量完成报告。
- **V0.8.0**：可授权全自动流水线——新增 19 号流水线收口门（初验→commit→同步基准分支并复验→push→PR→CI→合并→同步主工作区→部署→线上冒烟→确认后终态清理）；项目级挡位声明（手动挡保留全部确认护栏，自动挡须先补全流水线契约）；自动挡统一使用 worktree；最简 CI 模板。
...
14. **轻量路由与交付策略分层**：小改始终保持轻量规格、最小验收和 6 行轻量完成报告；自动挡只额外运行 03「自动挡双项轻量门禁」并走 19，流水线异常时才升级完整报告。
15. **可恢复状态**：流水线停止、跨窗口、等待清理或 DONE 时把可靠状态写入 PROJECT_STATE，恢复前以 Git / gh / CI / 部署平台事实复核。
16. **验证分级**：默认先跑 V1 终端自动化，网页交互按需升到 V2 浏览器自动化；只有前两级无法覆盖且本轮已授权时才运行 V3 Computer-Use，不在每个小步后重复控制电脑。
17. **执行规则收紧（V0.8.2）**：后端哨兵必须同时命中未验收后端与本轮后端业务范围；所有暂存均使用明确路径；UI 小改步骤只以 16 为准；同步基准分支后用 Git tree 区分沿用初验与完整复验。
18. **洁癖门独立（V0.9.0）**：项目洁癖三层知识编辑拆为独立 Skill `kun-cleanup-gate`，直接调用不再经 Router；Router 保留移交入口，两侧 description 划清边界。
...
> 「项目洁癖 / 收尾 / 同步文档」已独立为 `kun-cleanup-gate` Skill，直接说即可，不需要先进 Router。
...
```text
执行洁癖门：本轮功能已经验收通过。请核对项目文档、项目规则和项目记忆；需要删除或改其他项目时先列待我拍板。
```

对应路由：`16-task-routing-map.md`「项目收尾 / 项目洁癖 / 三层知识编辑」。

该流程已独立为 `kun-cleanup-gate` Skill，可直接调用。
...
> `references/13-project-cleanup-*.md` 是拆分前的原文件，已迁移至 `cleanup-gate/`，保留作回退参考；确认稳定后由用户手动移除。
