...
- 在 feature worktree 获取最新远程状态，并按项目契约选择 merge 或 rebase，同步 `<远程名>/<基准分支>`；无远程的本地项目则同步本地 `<基准分支>`。
- 有冲突立即停，解决冲突后必须重新跑 12 号完整验收。同步后代码发生过变化，旧验收结果不能直接沿用。
- 复验全绿、无「未实际验证」项且工作区干净后才能 push；失败则停在 `BASE_SYNCED`，记录失败项和恢复入口。
...
- 先汇报 PR、CI、合并、主工作区同步、部署和冒烟结果，并列出每个待清理对象的类型与绝对路径 / 分支名。
...
| 步骤 | 手动挡 | 自动挡 |
|---|---|---|
| Step 1-5 | 每个 Git / 上线动作先给建议，等用户确认 | 契约完整：初验后自动 commit，同步基准分支并复验全绿后继续连跑 |
| 汇报 | 照常 | 照常 |
...
不要因为 feature 分支已经验收，就跳过同步 `<基准分支>` 与复验；也不要在合并命令里顺手删除远程分支。正确顺序是：先按本门完成收口与汇报，再等用户确认精确清理清单。
