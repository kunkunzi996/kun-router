...
Router 应该走轻量路由：两种挡位都只走轻量规格与最小验收。手动挡给 Git 保存建议；自动挡额外运行 03 第 6/7 项并走 19，绝不恢复 Product Brief / AI-SDD / 架构门。
...
Router 应该启用：流水线收口（19）。自动挡项目在契约完整、同步基准分支并复验全绿后会自动走这一段；能力缺失时降级手动挡，终态清理前必须确认精确清单。
...
Router 应该启用：保存汇报 → 项目洁癖（三层体检、盘点、审计与同步）→ 如准备新窗口，Handoff Protocol；最后输出三层知识编辑报告。删除、全局规则和跨项目动作不会自动执行。
